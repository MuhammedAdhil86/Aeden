...
import { fetchIdleMonitioring, fetchInsuranceTracking, searchInsurance } from "@/service/report";
import { fetchTrip } from "@/service/fleet";
import DownloadInsurance from "./DownloadInsurance";
...
const columns = [
  ...
  {
    accessorKey: "insurance_policy_no",
    header: "Policy No.",
    cell: ({ getValue }) => getValue(),
  },
  {
    accessorKey: "insurance_premium_amount",
    header: "Policy Amount",
    cell: ({ getValue }) => `₹ ${getValue()}` || "₹ 0",
  },
  {
    accessorKey: "insurance_invoice_number",
    header: "Insurance Invoice No.",
    cell: ({ getValue }) => getValue(),
  },
  {
    accessorKey: "insurance_owner_name",
    header: "Insurance Owner name",
    cell: ({ getValue }) => getValue(),
  },
];
...
export default function page() {
 ...
 const fetchDefaultData = async () => {
    try {
      ...
      const response = await fetchInsuranceTracking(startDate, endDate);
      ...
      const tripsArray = response?.entries
        ? response.entries.map((data) => ({
            ...
            insurance_owner_name: data.insurance_owner_name || "N/A",
            insurance_premium_amount: data.insurance_premium_amount || "0",
            insurance_invoice_number: data.insurance_invoice_number || "N/A",
          }))
        : [];

      console.log("Insurance tracking data:", tripsArray);
      setData(tripsArray);
      setIsSearching(false);
    } catch (error) {
      console.error("Failed to fetch insurance tracking:", error);
      setData([]);
    }
  };
  ...
  const fetchSearchData = async (term) => {
    try {
      const response = await searchInsurance(term);
      ...
      console.log("Search insurance tracking data:", searchArray);
      setData(searchArray);
      setIsSearching(true);
    } catch (error) {
    ...
    <div>
      ...
      <div className="pt-5 px-2">
        <div className="w-full flex gap-x-5 items-center">
          ...
          <DownloadInsurance data={data} className="" />

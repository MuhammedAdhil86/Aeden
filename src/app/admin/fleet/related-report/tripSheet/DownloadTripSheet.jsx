...
function DownloadTripSheet({data}) {
  const handleDownload = () => {
    ...
    const exportData = data.map((item) => ({
      ...
      "Distance Traveled": item.dist || "0",
      "Trip Date": item.tripDate || "0",
    }));
    ...
    utils.book_append_sheet(workbook, worksheet, "Trip Sheet");

    // Generate file name with current date (e.g., fleet_expense_2025-05-19.xlsx)
    const today = format(new Date(), "yyyy-MM-dd");
    const fileName = `trip_sheet_${today}.xlsx`;
...
export default DownloadTripSheet;

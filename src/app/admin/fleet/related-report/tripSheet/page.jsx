...
import { filterTripLog, searchTripReport } from "@/service/report";
import DownloadTripSheet from "./DownloadTripSheet";
...
export default function TripSheetLogbook() {
  ...
  const fetchDefaultData = async () => {
    try {
      let response;
      if (fromDate && toDate) {
        // Use filterTripLog when both dates are selected
        const startDate = format(fromDate, "yyyy-MM-dd");
        const endDate = format(toDate, "yyyy-MM-dd");
        response = await filterTripLog(startDate, endDate);
      } else {
        // Use fetchTrip as default when no dates are selected
        response = await fetchTrip();
      }
      ...
      const tripsArray = response
        ? response.map((data) => ({
            ...
            tripDate: data.unloading_date
              ? format(new Date(data.unloading_date), "MMM dd, yyyy")
              : "N/A",
            driverName: data.driverid.full_name || "N/A",
            dist: data.total_kilometer || "0",
          }))
        : [];
  ...
  const fetchSearchData = async (term) => {
    try {
      const response = await searchTripReport(term);
      ...
      console.log("Search trips data:", searchArray);
      setData(searchArray);
      setIsSearching(true);
    } catch (error) {
    ...
    <div>
      ...
      <div className="pt-5 px-2">
        <div className="w-full flex gap-x-5 items-center">
          <p className="text-black font-medium pt-1">Trip Sheet Logbook</p>

...
import { searchService, fetchServiceLog } from "@/service/report";
...
const fetchDefaultData = async () => {
    try {
      ...
      const response = await fetchServiceLog(startDate, endDate);
      console.log("response : ", response);

      const tripsArray = response?.trucks
        ? response.trucks.map((data) => ({
            id: data.vehicle_id,
            regNo: data.vehicle_number || "",
            vehicleName: data.vehicle_name || "",
            serviceDate: data.last_updated_date
              ? format(new Date(data.last_updated_date), "MMM dd, yyyy")
              : "N/A",
            total_services: data.total_services || "0",
            cost: data.total_service_cost || "0",
          }))
        : [];
  ...
  const fetchSearchData = async (term) => {
    try {
      ...
      const searchArray = response?.trucks
        ? response.trucks.map((data) => ({
            id: data.vehicle_id,
            regNo: data.vehicle_number || "",
            vehicleName: data.vehicle_name || "",
      ...
      console.log("Search service log data:", searchArray);
      setData(searchArray);
      setIsSearching(true);
    } catch (error) {
    ...
    <div>
      ...
      <div className="pt-5 px-2">
        <div className="w-full flex gap-x-5 items-center">
          <p className="text-black font-medium pt-1">Service History Log</p>
          ...
          <DownloadService data={data} className=''/>
                  ...
                  <TableRow
                    ...
                    onClick={(e) => {
                      ...
                      if (!row.original.id) {
                       ...
                       router.push(`/admin/fleet/related-report/service/detail?id=${row.original.id}`);
                    }}
                  >

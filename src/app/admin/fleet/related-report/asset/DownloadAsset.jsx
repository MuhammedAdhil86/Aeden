...
function DownloadAsset({ data }) {
 const handleDownload = () => {
     ...
     const exportData = data.map((item) => ({
       ...
       "Last Maintenance Date": item.last_mnts_date || "0",
       "Total Maintenance": item.totalExpense || "0",
     }));
     ...
     utils.book_append_sheet(workbook, worksheet, "Asset Management");
 
     // Generate file name with current date (e.g., fleet_expense_2025-05-19.xlsx)
     const today = format(new Date(), "yyyy-MM-dd");
     const fileName = `Asset_Management_${today}.xlsx`;

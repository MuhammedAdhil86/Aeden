...
import { fetchFleetExpense, fleetSearch } from "@/service/report";
...
export default function ExpensePage() {
    ...
    try {
      ...
      const tripsArray = response
        ? response.map((data) => ({
            ...
            KmDriven: data.truckid.odometer_km || "0.00",
            totalExpense: data.trip_expenses || "0.00",
  ...
  const fetchSearchData = async (term) => {
    try {
      const response = await fleetSearch(term);
      ...
      console.log("Search expense data:", searchArray);
      setData(searchArray);
      setIsSearching(true);
    } catch (error) {
    ...
    <div>
      ...
      <div className="pt-5 px-2">
        <div className="w-full flex gap-x-5 items-center">
          ...
          <div className="flex w-52 justify-between rounded-l-lg">
            <div className="flex items-center">
              <Input
                ...
                value={searchTerm}
                className="px-3 py-2 bg-white text-black text-sm focus:ring-0 focus:ring-offset-0 focus-visible:ring-0 focus-visible:ring-offset-0"
              />
            </div>
              ...
              {table.getRowModel().rows.length ? (
                table.getRowModel().rows.map((row) => (
                  <TableRow
                    ...
                    onClick={(e) => {
                      console.log(
                        "Navigating to expense truck ID:",
                      ...
                      router.push(
                        `/admin/fleet/related-report/expense/detail?id=${row.original.id}`
                      );
                    }}
                  >

...
import { fetchIdleMonitioring, searchIdleMonitioring } from "@/service/report";
...
export default function page() {
  ...
  const fetchDefaultData = async () => {
    try {
      ...
      const response = await fetchIdleMonitioring(startDate, endDate);
      ...
      const tripsArray = response?.entries
        ? response.entries.map((data) => ({
            ...
            lastTripDate: data.last_trip_date
              ? format(new Date(data.last_trip_date), "MMM dd, yyyy")
              : "N/A",
            idlDays: data.idle_days || "0",
            status: data.status || "N/A",
          }))
        : [];

      console.log("Idle monitoring data:", tripsArray);
      setData(tripsArray);
      setIsSearching(false);
    } catch (error) {
      console.error("Failed to fetch idle monitoring data:", error);
      setData([]);
    }
  };
  ...
  const fetchSearchData = async (term) => {
    try {
      const response = await searchIdleMonitioring(term);
      ...
      console.log("Search idle monitoring data:", searchArray);
      setData(searchArray);
      setIsSearching(true);
    } catch (error) {
    ...
    <div>
      ...
      <div className="pt-5 px-2">
        <div className="w-full flex gap-x-5 items-center">
          ...
          <DownloadIdle data={data} className="" />

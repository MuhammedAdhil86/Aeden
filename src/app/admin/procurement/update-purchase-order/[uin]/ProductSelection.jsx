...
function ProductSelection({
  ...
  // Pre-fill all fields on component mount if selectedProducts exists
  useEffect(() => {
    if (selectedProducts.length > 0 && !watchedProduct) {
      const firstProduct = selectedProducts[0];
      setValue("product", firstProduct.product);
      console.log("Pre-filled product on mount:", firstProduct.product);
    }
  }, [selectedProducts, setValue, watchedProduct]);

  // Update fields when watchedProduct changes
  useEffect(() => {
    if (watchedProduct) {
      const selectedProduct = selectedProducts.find(
        (sp) => sp.product === watchedProduct
      );
      if (selectedProduct) {
        setValue("variety", selectedProduct.variety || "");
        setValue("brand", selectedProduct.brand || "");
        setValue("currency", selectedProduct.currency || "USD");
        setValue("unitPrice", selectedProduct.unitPrice || "");
        const quantitySpec = selectedProduct.specifications.find(
          (spec) => spec.type === "quantity"
        );
        const weightSpec = selectedProduct.specifications.find(
          (spec) => spec.type === "weight"
        );
        if (quantitySpec) {
          setValue("quantity", quantitySpec.quantity || "");
          setValue("countSize", quantitySpec.countSize || "");
          setValue("unit", quantitySpec.unit || "");
        } else {
          setValue("quantity", "");
          setValue("countSize", "");
          setValue("unit", "");
        ...
        if (weightSpec) {
          setValue("netWeight", weightSpec.netWeight || "");
          setValue("weightUnit", weightSpec.weightUnit || "");
        } else {
          setValue("netWeight", "");
          setValue("weightUnit", "");
        }
        setCurrentProductSpecs(selectedProduct.specifications || []);
        console.log("Pre-filled fields for product:", {
          product: watchedProduct,
          variety: selectedProduct.variety,
          brand: selectedProduct.brand,
          currency: selectedProduct.currency,
          unitPrice: selectedProduct.unitPrice,
          quantity: quantitySpec?.quantity,
          unit: quantitySpec?.unit,
          netWeight: weightSpec?.netWeight,
          weightUnit: weightSpec?.weightUnit,
          specifications: selectedProduct.specifications,
        });
      } else {
        // Clear fields if no matching product in selectedProducts
        setValue("variety", "");
        setValue("brand", "");
        setValue("currency", "");
        setValue("unitPrice", "");
        ...
        console.log("Cleared fields for product:", watchedProduct);
      }

      // Fetch varieties for the selected product
      const productObj = products.find(
        (p) => p.product_name === watchedProduct
      );
      if (productObj && productObj.id) {
        setLoadingVarieties(true);
        fetchVarityByProductId(productObj.id)
          .then((varietyData) => {
            let fetchedVarieties = varietyData?.data || [];
            const cachedVarieties = varietiesCache[productObj.id] || [];
            const uniqueVarieties = [
              ...cachedVarieties,
              ...fetchedVarieties.filter(
                (v) => !cachedVarieties.find((cv) => cv.name === v.name)
              ),
            ];
            setVarieties(uniqueVarieties);
            console.log(
              "Updated varieties for product",
              watchedProduct,
              ":",
              uniqueVarieties
            );
          ...
          .catch((error) => {
            ...
            setVarieties(varietiesCache[productObj.id] || []);
          })
  ...
  }, [watchedProduct, products, varietiesCache, selectedProducts, setValue]);

  // Debug selectedProducts, products, varieties, and units
  useEffect(() => {
    console.log("ProductSelection selectedProducts:", selectedProducts);
    console.log("ProductSelection products:", products);
    console.log("ProductSelection varietiesCache:", varietiesCache);
    console.log("ProductSelection varieties:", varieties);
    console.log("ProductSelection units:", units);
  }, [selectedProducts, products, varietiesCache, varieties, units]);
  ...
  const removeProduct = (productId) => {
    ...
    if (
      watchedProduct ===
      selectedProducts.find((p) => p.id === productId)?.product
    ) {
      setValue("product", "");
    }
  };
  ...
  const updateQuantity = (change) => {
    const currentQuantity = parseFloat(watchedQuantity) || 0;
    const newQuantity = Math.max(0, currentQuantity + change);
    setValue("quantity", newQuantity);
  };

  const updateNetWeight = (change) => {
    const currentWeight = parseFloat(watchedNetWeight) || 0;
  ...
  const handleAddProduct = async (data) => {
    try {
      ...
      setValue("product", data.product_name); // Pre-select new product
    ...
    try {
      ...
      if (watchedProduct) {
        ...
        if (productObj && productObj.id === data.product_id.id) {
          setVarieties((prev) => {
            const updatedVarieties = [...prev, newVariety];
            setVarietiesCache((prevCache) => ({
              ...prevCache,
              [productObj.id]: updatedVarieties,
            }));
            return updatedVarieties;
          });
          setValue("variety", data.name); // Pre-select new variety
        }
      }
  ...
  const handleAddUnit = async (data) => {
    try {
      ...
      setUnits((prev) => {
        ...
        console.log("Updated units after adding:", updatedUnits);
        return updatedUnits;
      });
      setValue("unit", data.unit_name); // Pre-select new unit
        ...
        <div className="grid grid-cols-[25.5%_23%_23%_23%] gap-4 border-b border-b-neutral-200 pb-6 mb-6">
          <div>
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => {
                console.log("ProductSelection product field:", field);
                return (
                  <Select
                    onValueChange={(value) => {
                      if (value === "add_new") {
          ...
          <div>
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => {
                console.log("ProductSelection variety field:", field);
                return (
                  <Select
                    onValueChange={(value) => {
                      if (value === "add_new") {
          ...
          <div>
            <p className="text-[13px] ps-5 pb-1">Brand (Optional)</p>
            <Input
              {...register("brand")}
          ...
          <div className="flex gap-x-5 items-end">
                ...
                <div className="h-8 bg-black flex flex-col rounded-r-lg justify-center">
                  ...
                  >
                    <svg
                      ...
                      fill="#FFFFFF"
                    >
                      <path d="M480-344 240-584l-56-56 184 184 184-184 56 56-240 240Z" />
                    </svg>
                  </div>
                </div>
              </div>
            ...
            <div className="w-1/4">
              ...
              <Controller
                ...
                name="unit"
                render={({ field }) => {
                  console.log("ProductSelection unit field:", field);
                  return (
                    <Select
                      value={field.value || ""}
                      onValueChange={(value) => {
        ...
        <div className="grid grid-cols-[25%_25%_25%_auto] gap-4 border-b border-b-neutral-200 pb-6 mb-6">
          <div>
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => {
                console.log("ProductSelection currency field:", field);
                return (
                  <Select
          ...
          <div>
            ...
            <div className="border border-neutral-400 rounded-lg px-3 py-1.5 bg-gray-50">
              <span className="text-xs font-medium">
                {watch("currency") || "USD"} {calculateTotalAmount()}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="flex gap-x-5 items-end">
            ...
            <div className="w-1/4">
              ...
              <Controller
                ...
                name="weightUnit"
                render={({ field }) => {
                  console.log("ProductSelection weightUnit field:", field);
                  return (
                    <Select
                      value={field.value || ""}
                      onValueChange={field.onChange}

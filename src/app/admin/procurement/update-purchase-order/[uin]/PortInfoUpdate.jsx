...
import { createPort, createCountry } from "@/service/procurements";

// CountryDialog Component
const CountryDialog = ({ open, onClose, onSave }) => {
  ...
  const onCountrySubmit = async (data) => {
...
const PortDialog = ({ open, onClose, onSave, countries, setIsCountryDialogOpen }) => {
  const {
...
// PortInfoUpdate Component
function PortInfoUpdate({ control, ports, setPorts, setValue, errors, countries, setCountries, register, purchase }) {
  ...
  useEffect(() => {
    if (purchase) {
      console.log("Purchase data in UpdatePortInfo:", purchase);
      ...
      console.log("No purchase data provided to UpdatePortInfo");
    }
  }, [purchase, setValue]);
  ...
  const handleAddCountry = async (data) => {
    try {
      ...
      //dialogControl._formValues.country_id = String(newCountry.id);
    } catch (error) {
      toast.error(`Failed to add country: ${error.message}`);
      throw error;
    }
    ...
    <div>
      ...
      <div className="grid grid-cols-[25.5%_23%_auto] gap-7 border-b border-b-neutral-200 pb-10">
        ...
        <PortDialog
          ...
          countries={countries}
          setIsCountryDialogOpen={setIsCountryDialogOpen}
        />
        <CountryDialog
